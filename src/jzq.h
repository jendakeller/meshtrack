...
struct Mat
{
  ...
  Mat<M,N,T>();
  template<typename T2> explicit Mat<M,N,T>(const Mat<M,N,T2>& B);
...
template<int M,int N,typename T> template<typename T2>
Mat<M,N,T>::Mat(const Mat<M,N,T2>& B)
{
  for (int i=0;i<M;i++)
  for (int j=0;j<N;j++)
  {
    m[i][j] = static_cast<T>(B.m[i][j]);
  }
}
